...
window.FaqSearchEngine = class FaqSearchEngine {
    constructor() {
        this.index = [];
        this.cache = new Map(); // Bounded cache for performance
        this.buildIndex();
    }
    ...
    search(query) {
        ...
        // --- CACHE MEMOIZATION ---
        // Instantly return previously computed results. This drops execution time
        // for repeated queries significantly.
        ...
        // Use traditional for loop instead of map/filter to avoid excessive
        // temporary array allocation and object spreading (...item).
        for (let i = 0; i < this.index.length; i++) {
            ...
            if (score > 0) {
                // Manually map fields to avoid expensive object spread (...item)
                results.push({
                    type: item.type,
                    id: item.id,
                    catId: item.catId,
